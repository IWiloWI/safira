...
import { MultilingualText, Product } from '@/types/database';
...
class TranslationService {
  ...
  private translationCache: Map<string, MultilingualText> = new Map();
  ...
  private async loadCache(): Promise<void> {
    try {
      ...
      const cacheObj: TranslationCache = JSON.parse(cacheData);
  ...
  private async saveCache(): Promise<void> {
    try {
      const cacheObj: TranslationCache = Object.fromEntries(this.translationCache);
  ...
  public async translateText(text: string, targetLanguages: string[] = ['da', 'en']): Promise<MultilingualText> {
    ...
    try {
      ...
      const targetLanguageList = targetLanguages.map(lang => languageNames[lang] || lang).join(' and ');
...
Please respond in JSON format like this:
{
  ...
  "en": "English translation here"
      ...
      const translations: Record<string, string> = JSON.parse(response.trim());
      
      // Validate that we got the expected languages
      const result: MultilingualText = { de: text };
      
      targetLanguages.forEach(lang => {
        if (translations[lang]) {
          if (lang === 'da') {
            result.da = translations[lang];
          } else if (lang === 'en') {
            result.en = translations[lang];
          }
        }
      });
  ...
  public async translateProductName(name: string, targetLanguages: string[] = ['da', 'en']): Promise<MultilingualText> {
...
Please respond in JSON format like this:
{
  ...
  "en": "English translation here"
      ...
      const result: MultilingualText = { de: name };
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      throw new Error(`Product name translation failed: ${errorMessage}`);
    }
  }

  public async autoTranslateProduct(
    product: Product, 
    categoryId: string | null = null, 
    translationOptions: TranslationOptions | null = null
  ): Promise<Product> {
    if (!this.isConfigured()) {
